...
class ApiDealsVoucherWebviewController extends Controller
{
    ...
    public function detailVoucher(Request $request)
    {
        ...
        if($request->type_deals=='voucher'){
            $post['id_deals_user'] = $request->id_deals;
        }else{
            $post['id_deals'] = $request->id_deals;
        }
        $post['used'] = 0;
        ...
        if($request->type_deals=='voucher'){
            ...
            ->where('id_deals_user', $request->id_deals)
            ->get()
            ->toArray()[0]??null;
            ...
            $result = [
                ...
                'confirmation_text'     => $data['is_used'] ? 'Batalkan penggunaan <b>' . $data['deals_voucher']['deal']['deals_title'] . '</b>' : '<b>' . $data['deals_voucher']['deal']['deals_title'] . '</b> akan digunakan pada transaksi selanjutnya',
                'popup_message'         => [
            ...
            if (!$deal) {
                return [
                    'status' => 'fail',
                    'messages' => ['Deal is not found']
                ];
            }

            if( $deal['is_all_outlet'] == 1 && isset($deal['id_brand']) ){
                $outlets = Outlet::join('brand_outlet', 'outlets.id_outlet', '=', 'brand_outlet.id_outlet')
                    ->join('deals', 'deals.id_brand', '=', 'brand_outlet.id_brand')
                    ->where('deals.id_deals', $deal['id_deals'])
                    ->where('outlet_status', 'Active')
                    ->select('outlets.*')->with('city')->get()->toArray();
                $deal['outlets'] = $outlets;
            }
            elseif ($deal['is_all_outlet'] == 1 && isset($deal['deals_brands'])) {
                $list_outlet = array_column($deal['deals_brands'], 'id_brand');
                $outlets = Outlet::join('brand_outlet', 'outlets.id_outlet', '=', 'brand_outlet.id_outlet');
            ...
            if (!empty($deal['outlet_groups'])) {
                $deal['outlets'] = app($this->deals_webview)->getOutletGroupFilter($deal['outlet_groups'], $deal['deals_brands'], $deal['brand_rule']);
            }
    
            if (!empty($deal['outlets'])) {
                $kota = array_column($deal['outlets'], 'city');
                $kota = array_values(array_map("unserialize", array_unique(array_map("serialize", $kota))));
    
                foreach ($kota as $k => $v) {
                    if ($v) {
                        $kota[$k]['outlet'] = [];
                        foreach ($deal['outlets'] as $outlet) {
                            if ($v['id_city'] == $outlet['id_city']) {
                                unset($outlet['pivot']);
                                unset($outlet['city']);
                ...
                $deal['outlet_by_city'] = $kota;
            }
    
            usort($deal['outlet_by_city'], function($a, $b) {
                if(isset($a['city_name']) && isset($b['city_name'])){
                    return $a['city_name'] <=> $b['city_name'];
                }
            });
    
            for ($i = 0; $i < count($deal['outlet_by_city']); $i++) {
                usort($deal['outlet_by_city'][$i]['outlet'] ,function($a, $b) {
                    return $a['outlet_name'] <=> $b['outlet_name'];
                });
            }
    
            $deal['deals_image'] = config('url.storage_url_api') . $deal['deals_image'];
    
            $data = $deal;
            $result = [
                'deals_image'           => $data['deals_image'],
                'deals_title'           => $data['deals_title'],
                'deals_second_title'    => $data['deals_second_title'],
                'deals_description'   	=> $data['deals_description'],
                'custom_outlet_text'    => $data['custom_outlet_text'],
                'is_used'               => 0,
                'btn_used'              => 'Gunakan Nanti',
                ...
                'header_offline_voucher'=> 'Offline Transaction',
                'title_offline_voucher' => 'Redeem directly at Cashier',
                'button_text'           => 'KLAIM DAN GUANAKAN',
                'confirmation_text'     => $data['deals_title'] . '</b> akan diklaim lalu digunakan pada transaksi selanjutnya',
                'popup_message'         => [
                    $data['deals_title'],
                    'akan digunakan pada transaksi selanjutnya'
                ],
            ...
            $i = 0;
            foreach ($data['deals_content'] as $keyContent => $valueContent) {
                if (!empty($valueContent['deals_content_details'])) {
                    $result['deals_content'][$i]['title'] = $valueContent['title'];
                    foreach ($valueContent['deals_content_details'] as $key => $value) {
                        $result['deals_content'][$i]['detail'][$key] = $value['content'];
            ...
            $result['deals_content'][$i]['is_outlet'] = 1;
            $result['deals_content'][$i]['title'] = 'Tempat Penukaran';
            $result['deals_content'][$i]['brand'] = $data['brand']['name_brand'] ?? '';
            $result['deals_content'][$i]['brand_logo'] = $data['brand']['logo_brand'] ?? '';
    
            if($data['custom_outlet_text'] != null){
                $result['deals_content'][$i]['detail'][] = $data['custom_outlet_text'];
            }else{
                foreach ($data['outlet_by_city'] as $keyCity => $valueCity) {
                    if (isset($valueCity['city_name'])) {
                        $result['deals_content'][$i]['detail_available'][$keyCity]['city'] = $valueCity['city_name'];
                        foreach ($valueCity['outlet'] as $keyOutlet => $valueOutlet) {
                            $result['deals_content'][$i]['detail_available'][$keyCity]['outlet'][$keyOutlet] = $valueOutlet['outlet_name'];
