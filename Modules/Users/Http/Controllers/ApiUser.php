...
class ApiUser extends Controller
{
    ...
    {
        $post = $request->json()->all();
        if (isset($post['order_field'])) $order_field = $post['order_field'];
        else $order_field = 'id';
        if (isset($post['order_method'])) $order_method = $post['order_method'];
        else $order_method = 'desc';
        if (isset($post['skip'])) $skip = $post['skip'];
        else $skip = '0';
    ...
    public function updateSuspendByAdmin(Request $request)
    {
        ...
        $dtUpdate['is_suspended'] = $post['is_suspended'];
        if($post['is_suspended'] == 1){
            $dtUpdate['suspended_date'] = date('Y-m-d H:i:s');
        }else{
            $dtUpdate['suspended_date'] = NULL;
        }
        $update = User::where('phone', $post['phone'])->update($dtUpdate);
