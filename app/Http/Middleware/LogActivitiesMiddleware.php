	...
	$response = $next($request);
        ...
        if(!isset($arrReq['log_save']) || ($arrReq['log_save'] == true && $arrReq['log_save'] == 'true')){
            ...
            if(!isset($arrReq['page']) || (int)$arrReq['page'] <= 1){
                ...
                if(stristr($url, 'pos/order/accept')) $subject = 'POS Order Accept';
                if(stristr($url, 'pos/order/ready')) $subject = 'POS Order Ready';
                if(stristr($url, 'pos/order/taken')) $subject = 'POS Order taken';
                if(stristr($url, 'pos/order/reject')) $subject = 'POS Order reject';
